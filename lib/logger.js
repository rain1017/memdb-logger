...
function getLogger(categoryName) {
    var prefix = arguments.length > 1 ? '[' + [].slice.call(arguments, 1).join('] [') + '] ' : '';
	...
	for (var key in logger) {
		mdbLogger[key] = logger[key];
	}

    var rawMessage = process.env.RAW_MESSAGE;
    var loggerLine = process.env.LOGGER_LINE;

	['log', 'debug', 'info', 'warn', 'error', 'trace', 'fatal'].forEach(function(item) {
		mdbLogger[item] = function() {
			if (!rawMessage) {
				var p = prefix;
				...
				// get transaction id from domain
	            if(process.domain && process.domain.__memdb__){
	                p += '[trans:' + process.domain.__memdb__.trans + '] ';
	            }
